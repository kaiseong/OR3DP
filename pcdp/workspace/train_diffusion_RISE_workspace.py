...
class TrainRISEWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        # IMPORTANT: When resuming from checkpoint, keep the existing normalizer
        # to avoid mismatch between model weights and normalizer range
        if cfg.training.resume and self.global_step > 0:
            # Use normalizer from checkpoint (already loaded with model)
            cprint("✓ Using normalizer from checkpoint (resume mode)", "green")
            normalizer = self.model.normalizer
            # Make sure val_dataset uses the same normalizer
        else:
            # Fresh training: create normalizer from dataset
            ...
            cprint("✓ Created new normalizer from dataset", "green")
